        ...
        <div className="flex items-center justify-between bg-[#77B6FF] py-2.5 px-4 mb-8 rounded-xl text-white">
            ...
            <div className="flex flex-col items-start">
                ...
                <span className="text-white text-xs font-bold">
                    {examinerName
                        ? `Penguji : ${examinerName}`
                        : "Penguji : Dr Mafkar Afkar"}
                </span>
            </div>
            <div
    ...
    return (
        <div className="flex text-[15px] font-semibold text-black bg-[#C4C6CD] mb-4 rounded-xl items-center border border-solid border-black">
            {columns.map((column, index) => (
                <div
                    key={index}
                    className={`${column.width || "flex-1"} flex ${
                        column.classes || "justify-center items-center"
                    }
           ${
...
const OsAssessmentRow = ({ columns = [], isBold = false }) => {
    const baseStyle =
        "flex text-[15px] items-center text-black border border-solid border-black mb-4 rounded-xl min-h-[48px]";
...
const StaseAssessmentView = ({
    ...
    const headerColumns = [
        ...
        {
            content: "Aspek Penilaian (5 Bobot)",
            width: "flex-1",
            classes: "justify-start pl-2.5 font-bold",
        },
    ];
    ...
    const totalScore = assessmentData.reduce((sum, row) => {
        const score = parseInt(row.value.split(" ")[0], 10); // <-- CRASH DI SINI
    ...
    return (
        <div>
            ...
            <OsAssessmentRow columns={footerColumns} isBold={true} />
        </div>
    );
};

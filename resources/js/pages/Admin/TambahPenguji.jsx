  ...
  return (
    <div className="min-h-screen flex flex-col bg-os-white rounded-lg p-4">
      <header className="bg-white border-b border-gray-300 px-3 py-3 flex items-center justify-between gap-3">
        <Link href="/admin/penguji" className="bg-red-600 text-white p-3 rounded-xl border border-black hover:bg-red-500">
          <X size={20} />
        </Link>

        <div className="flex-1 mx-3 border border-black rounded-xl px-4 py-2 bg-white">
          <p className="text-black text-base sm:text-lg truncate">Penguji / {isEditMode ? " Edit Penguji" : " Tambah Penguji"}</p>
        </div>
      </header>
      ...
      <main className="flex-1 flex justify-center items-center sm:items-center p-4 sm:p-10">
        <div className="w-full sm:max-w-md bg-white border border-gray-700 rounded-xl shadow-md overflow-hidden">
          <div className="bg-neutral-800 text-white text-center py-6 px-4">
            <h2 className="text-xl font-semibold">Form Penguji</h2>
            <p className="text-sm text-gray-300 mt-1">Form ini berisi semua data yang digunakan untuk membuat penguji</p>
          </div>

          <form onSubmit={handleSubmit} className="p-6 space-y-3">
            <div>
              <label className="block text-xs text-gray-700 font-semibold mb-1">NIP penguji</label>
              <input
                value={data.nip}
            ...
            <div>
              <label className="block text-xs text-gray-700 font-semibold mb-1">Nama penguji</label>
              <input
                value={data.nama}
            ...
            <div>
              <label className="block text-xs text-gray-700 font-semibold mb-1">Alamat penguji</label>
              <textarea
                value={data.alamat}
            ...
            <div>
              <label className="block text-xs text-gray-700 font-semibold mb-1">Email penguji</label>
              <input
                value={data.email}
            ...
            <div className="flex flex-col sm:flex-row justify-between gap-3 pt-4">
              ...
              >
                <Save size={18} />
                {processing ? "Menyimpan..." : isEditMode ? "Perbarui" : "Submit"}
              </button>
